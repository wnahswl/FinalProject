...
public class NaverService implements SocialOauth {
	...
	public String getOauthRedirectURL(HttpSession session) {
		...
		log.info("paramString : {} ", parameterString);
		return redirectURL;
	}
	
	public void getRemoveTokenRUL(String token) throws IOException {
		...
		//parameter를 형식에 맞추는 함수
		String parameterString = params.entrySet().stream().map(x->x.getKey() + "=" + x.getValue())
				.collect(Collectors.joining("&"));
		String deleteTokenURL = NAVER_SNS_TOKEN_URL+"?"+parameterString;
		log.info("delete token request url : {}  ", deleteTokenURL);
		
		response.sendRedirect(deleteTokenURL);
	}
